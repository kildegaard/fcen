        ...
        for lineas in datos:
            for indice in range(len(datos[0]) - int(tamVentana) + 1):
                ...
                if metodo == 'def' or metodo == '':
                    ...
                    cantNA = datosSalidaAux.count('NA')
                    if cantNA == 0:                             # Si ninguno es NA, hago el promedio tranqui
                        datosSalida[numLinea][indice] = prom(datosSalidaAux)
                    elif cantNA == len(datosSalidaAux):         # Si todos son NA, pongo 'NA'
                        datosSalida[numLinea][indice] = 'NA'
                    else:
                        while hayNA(datosSalidaAux):
                            datosSalidaAux.remove('NA')
                        datosSalida[numLinea][indice] = prom(datosSalidaAux)
