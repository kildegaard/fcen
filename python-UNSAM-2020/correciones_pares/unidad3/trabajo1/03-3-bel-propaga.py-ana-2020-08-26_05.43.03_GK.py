...
def propagar(lista):
    propagada = []
    # Com: Nombre de variables poco explicativas
    for n, i in enumerate(lista):  # Con el enumerate me guardo las posiciones
        # Com: Hacer esto con try-except me parece un approach equivocado
        try:
            if i == 1:  # Si es 1 que guarde su valor
                propagada.append(lista[n])
                if lista[n-1] == 0:  # si a la izquierda del 1 hay 0
                    # Com: esta forma de "guardar un 1" es bastante confusa, no la recomiendo
