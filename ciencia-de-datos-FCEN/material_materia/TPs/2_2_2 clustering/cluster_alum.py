# -*- coding: utf-8 -*-
"""
Created on Fri Oct 25 19:06:05 2019
...
#%%
#GK: Cargamos la imagen y la mostramos por pantalla
...
#z = reshape(x,3,4); should become z = x.reshape(3,4,order=’F’)
r=np.double(a.reshape(fil*col,1,order='F'))
g=np.double(b.reshape(fil*col,1,order='F'))
b=np.double(L.reshape(fil*col,1,order='F'))
...
kmeans = KMeans(n_clusters=2, random_state=0).fit(H)

fig = plt.figure()
ax3D = fig.add_subplot(111, projection='3d')
ax3D.scatter(H[:,0],H[:,1],H[:,2], s=10, c=kmeans.labels_, marker='o')  
plt.show()
